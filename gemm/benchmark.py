...
def check_sparse_mm_dense(device):
    for i in range(16):
        ...
        with torch.no_grad():
            wrapper.assert_equal(wrapper.functional.sparse_mm_dense(sparse, dense), sparse.mm(dense), 1e-5)
        ...
        sparse.grad.zero_()
        dense.grad.zero_()
        wrapper.assert_equal(sparse_grad1, sparse_grad2, 1e-5)
        wrapper.assert_equal(dense_grad1, dense_grad2, 1e-5)  # TODO 报错，原因未知
