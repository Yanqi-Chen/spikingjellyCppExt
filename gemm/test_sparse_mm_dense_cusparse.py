...
cext = cpp_extension.load(name='sparse_mm_dense_cusparse', sources=['./gemm.cpp', './gemm.cu'], verbose=True)
...
device = 'cuda:0'
batch_size_list = list(range(16, 128 + 1, 16))
batch_size_list.reverse()
in_features_list = list(range(128, 8192 + 1, 128))
in_features_list.reverse()
out_features_list = list(range(128, 8192 + 1, 128))
...
real_sparsity = np.zeros(
    shape=[batch_size_list.__len__(), in_features_list.__len__(), out_features_list.__len__(), sparsity_list.__len__()])
print(t_sparse.shape, t_sparse.size)
    ...
    for i in range(t_sparse.shape[0]):
        for j in range(t_sparse.shape[1]):
            for k in range(t_sparse.shape[2]):
                ...
                if idx % 128 == 0:
                    print('\n', idx, i, j, k, batch_size, in_features, out_features)
                ...
                t2 = cal_fun_t(16, torch.mm, x, w)
                t_dense[i][j][k] = t2
                # print(t2, end=' | ')
                for l in range(t_sparse.shape[3]):
                    sparsity = sparsity_list[l]
                    spike = (x > sparsity).float()
                    real_sparsity[i][j][k][l] = 1 - spike.mean().item()
                    t1 = cal_fun_t(16, cext.sparse_mm_dense_cusparse, spike, w, y)
