...
std::vector<at::Tensor> LIF_hard_reset_fptt(torch::Tensor & x, torch::Tensor & v, const float & v_th, const float & v_reset, 
    const float & tau)
{
    CHECK_TENSOR(x);
    CHECK_TENSOR(v);
    ...
    CHECK_TENSOR(spike);
    CHECK_TENSOR(v_next)
    LIF_hard_reset_fptt_cuda(x.data_ptr<float>(), h.data_ptr<float>(), spike.data_ptr<float>(), v_next.data_ptr<float>(), 
        v_th, v_reset, x.size(0), x.numel(), x.get_device(),
        tau);
    return {h, spike, v_next};
}
