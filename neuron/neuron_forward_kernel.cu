...
__global__ void LIF_hard_reset_fptt_cuda_kernel(
  const float* __restrict__ x, float* __restrict__ h,  float* __restrict__ spike, float* __restrict__ v_next, 
  ...
  const float reciprocal_tau, const float one_sub_reciprocal_tau)
{
  ...
  if (index < neuron_num)
  {
    ...
    {
      h[index] = one_sub_reciprocal_tau * v_next[index] + reciprocal_tau * (x[index + mem_offset] + v_reset);
...
void LIF_hard_reset_fptt_cuda(const float* x, float* h, float* spike, float* v_next, 
  ...
  const float & tau)
{
  ...
  const float reciprocal_tau = 1 / tau;
  LIF_hard_reset_fptt_cuda_kernel<<<blocks, threads>>>(x, h, spike, v_next, v_th, v_reset, neuron_num, size, reciprocal_tau, 1 - reciprocal_tau);
}